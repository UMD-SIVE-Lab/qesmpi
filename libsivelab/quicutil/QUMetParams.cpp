...
bool quMetParams::readQUICFile(const std::string &filename)
{
  ...
  // 
  // We need to handle QUIC 5.6 and 5.72
  // 
  // Only difference is the header (5.6 or 5.72)

  if (quicVersion != "5.72" && quicVersion != "5.6")
  {
    std::cerr << "Error!  Exiting!  Only QUIC version 5.6 and 5.72 currently supported!  Version in current file set: " << quicVersion << std::endl;
    exit(EXIT_FAILURE);
  }
  ss.clear();
