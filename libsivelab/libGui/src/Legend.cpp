...
void Legend::createText(){
	...
	minVal.SetScale(.5, .5);
	minVal.SetPosition( m_posX+label_paddingL, m_posY+180 );
	...
	midVal.SetScale(.5, .5);
	midVal.SetPosition( m_posX+label_paddingL, m_posY+90 );
...
void Legend::newCreateGradient(){
	...
	if( m_optTracker->getActive( "colormode" ) ){
		...
		for( float i=0; i<m_colors.size(); i++ ){
			imgGradient.insert( i, m_colors[i] );
		}
	}
	else{
		sf::Color gray1 = sf::Color( 255, 255, 255, transparency );
		sf::Color gray2 = sf::Color( 128, 128, 128, transparency );
		sf::Color gray3 = sf::Color( 0, 0, 0, transparency );

		imgGradient.insert(0.0, gray1);
		imgGradient.insert(1.0, gray2);
		imgGradient.insert(2.0, gray3);
	}
	...
	for( int i=0; i < gWidth; i++ ){
		for( int j=0; j < gHeight; j++ ){
			gradientImage.SetPixel( i, j, tab[j] );
		}		
	}
