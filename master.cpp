...
namedOptParam *master::lookupDataInSolverMap(unsigned int idx)
{
...
namedOptParam *master::lookupDataInRangeMap(unsigned int idx)
{
    if (idx >= 0 && idx < rangeOptMap.size())
        return &(rangeOptMap[idx]);
    else
...
namedOptParam *master::lookupDataInSingleMap(unsigned int idx)
{
...
namedOptParam *master::lookupDataInSetMap(unsigned int idx)
{
...
std::string master::searchForPROJFile(const std::string &dir)
{
...
void master::copyFile(const std::string &sourceFilename, const std::string &destFilename)
{
    int length;
    char *byteBuffer;
...
directory workDir;
double master::eval_gpuPlume_sample_fitness( sample &s )          ///languageMap
{
    ////Fitness Values :
        ...
        for (unsigned int tsIdx = 0; tsIdx < timeStepSet.size(); tsIdx++)
            for (unsigned int npIdx = 0; npIdx < numParticleSet.size(); npIdx++)
                for (unsigned int avgParam_index = 0; avgParam_index < avgParam_Iterations; avgParam_index++)
                {
                    ...
                    std::cout << "Starting work on problem instance " << probInstID << " of Problem #" << currProbID << std::endl;
                    std::cout << "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=" << std::endl;
                    ...
                    */
                    std::cout << "before modifying dependency" << std::endl;
                    std::cout << "---------------------_" << std::endl;
                    std::cout << "quqpData.quBuildingData.x_subdomain_sw = " << quqpData.quBuildingData.x_subdomain_sw << std::endl;
                    std::cout << "quqpData.quBuildingData.y_subdomain_sw = " << quqpData.quBuildingData.y_subdomain_sw << std::endl;
                    std::cout << "quqpData.quBuildingData.x_subdomain_ne = " << quqpData.quBuildingData.x_subdomain_ne << std::endl;
                    ...
                    std::cout << "done with changing the buildings params . Only that should change nothing else and before gpuPlume " << std::endl;
                    if (testing_day)
                        std::cin >> pause_temp1;
